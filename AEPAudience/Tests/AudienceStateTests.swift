...
class AudienceStateTests: XCTestCase {
    ...
    func updatePrivacy(status: PrivacyStatus, shouldUpdateSharedState: Bool = false, shouldSendOptOutHit: Bool = false) {
        ...
        audienceState.handlePrivacyStatusChange(event: configEvent, createSharedState: { (_, _) in
            if !shouldUpdateSharedState {
                XCTFail("Shared state should not be updated")
            }
        }, dispatchOptOutResult: { (optedOut, _) in
    ...
    func testHandleHitResponseHappy() {
        ...
        audienceState?.handleConfigurationSharedStateUpdate(event: AudienceStateTests.configEvent, configSharedState: AudienceStateTests.configSharedState, createSharedState: { _, _ in
    ...
    func testHandleHitResponseWithEmptyConfigurationSettingsInAudienceState() {
        ...
        audienceState?.handleConfigurationSharedStateUpdate(event: AudienceStateTests.configEvent, configSharedState: [:], createSharedState: { _, _ in
        }, dispatchOptOutResult: { (_, _) in})
