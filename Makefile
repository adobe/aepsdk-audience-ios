...
test:
	...
	@echo "######################################################################"
	xcodebuild test -workspace $(PROJECT_NAME).xcworkspace -scheme $(AEPAUDIENCE_TARGET_NAME) -destination 'platform=iOS Simulator,name=iPhone 8' -enableCodeCoverage YES
